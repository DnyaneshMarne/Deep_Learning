# -*- coding: utf-8 -*-
"""
Created on Fri Mar 22 08:27:24 2022
...
from model_train import MODELS, encoderRNN, decoderRNN, attention
from preprocess_data import data_pro_test
...
def generateCaption(test_loader):
    ...
    model.eval()
    final_out = []
    with open('index_to_words.json', 'r') as fp:
        index_to_words = json.load(fp)
        
    index_to_words = {int(k):v for k,v in index_to_words.items()}
    for batch_idx, batch in enumerate(test_loader):
        ...
        result = [[index_to_words[x.item()] if index_to_words[x.item()] != '<UNK>' else 'something' for x in s] for s in caption_predictions]
