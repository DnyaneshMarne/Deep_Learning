# -*- coding: utf-8 -*-
"""
Created on Thu Mar 20 19:29:07 2022
...
from preprocess_data import data_pro_train,createMiniBatch
...
with open('index_to_words.json', 'w') as fp:
    json.dump(dataTrain.index_to_words, fp)
...
class decoderRNN(nn.Module):
    def __init__(self, hidden_size, output_size, vocab_size, word_dim, dropout_percentage=0.3):
        ...
        # layers
        self.embedding = nn.Embedding(len(dataTrain.index_to_words) + 4, 1024)
        self.dropout = nn.Dropout(0.3)
...
if __name__ == "__main__":
   # stuff only to run when not called via 'import' here
    ...
    encoder = encoderRNN()
    decoder = decoderRNN(512, len(dataTrain.index_to_words) +4, len(dataTrain.index_to_words) +4, 1024, 0.3)
